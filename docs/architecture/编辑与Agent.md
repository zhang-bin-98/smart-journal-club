...
计划编辑支持章节/讲述片段/页面的新增、更新、删除、重排及主线/补充调整，不保留 set-slide-budget 命令。移动章节带其页面与讲稿；仅调整页面展示顺序保留章节和片段身份，显式改变所属章节或跨章合页时迁移其已分配片段；仅显式删除章节时处理空章，禁止悬空 sectionId/speechIds。页间顺序由稳定锚点表示，null 为目标章首，拒绝不存在目标、自身后移或超范围变更。
