...
UI 中的 controller hook 仅绑定用例、订阅任务/内容变化并持有交互草稿；启动后流程可以脱离 React 执行。项目关闭须经过保存/放弃检查后明确取消任务，普通组件卸载或视图切换不关闭项目；不能让重新 render 创建新 workflow、会话或提交。固定流程使用普通显式函数，交互助手沿用 pi-agent-core，通过基础设施适配器创建 Agent 并转换事件，app/assistant 定义目标、工具回调与提案编排；两者共享用例权限和提交入口，不共享一套自由 Agent loop。
