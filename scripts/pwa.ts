  ...
  return {
    ...
    async buildStart() {
      ...
      for (const group of ['cmaps', 'standard_fonts', 'wasm']) {
        ...
        for (const name of await readdir(directory))
          this.emitFile({
            type: 'asset',
            fileName: `pdfjs/${group}/${name}`,
            source: await readFile(resolve(directory, name)),
          });
      }
    },
    configureServer(server) {
      ...
      server.middlewares.use(`${base}pdfjs/`, (req, res, next) => {
        ...
        if (!/^\/(cmaps|standard_fonts|wasm)\/[a-zA-Z0-9_.-]+$/.test(path)) {
          next();
          return;
        }
        void readFile(resolve(config.root, 'node_modules/pdfjs-dist', path.slice(1))).then(
          (data) => {
            res.setHeader(
              'Content-Type',
              path.endsWith('.wasm')
    ...
    async closeBundle() {
      ...
      const source = template
        .replace('__SMARTJC_VERSION__', JSON.stringify(hash.digest('hex').slice(0, 16)))
        .replace('__SMARTJC_FILES__', JSON.stringify(files))
        .replace('__SMARTJC_INTEGRITY__', JSON.stringify(integrity));
