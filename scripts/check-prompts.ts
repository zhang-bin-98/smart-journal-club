...
import { parsePromptFiles } from '../src/infrastructure/llm/promptConfig.ts';
const files: Record<string, string> = { 'prompts/common.md': await readFile('prompts/common.md', 'utf8') };
for (const directory of ['research', 'stages'])
  for (const name of await readdir(`prompts/${directory}`)) {
    if (name.endsWith('.md')) {
      const path = `prompts/${directory}/${name}`;
