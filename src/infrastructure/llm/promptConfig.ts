...
import type { ResearchStrategy, PromptCatalog } from '../../app/llm/promptCatalog';
export function parsePromptFiles(files: Record<string, string>): PromptCatalog {
  ...
  for (const [path, text] of Object.entries(files)) {
    ...
    if (research) {
      ...
      if (document.errors.length) throw new Error(`${path} YAML 无效`);
      const header = HeaderSchema.parse(document.toJS());
      ...
      if (!text.trim() || stages[stage[1]]) throw new Error(`${path} 阶段提示词为空或重复`);
      stages[stage[1]] = text.trim();
