import { get, request, transaction, type Store } from '../../shared/persistence/indexedDb';
import { openProject, projectIn, paperIn, projectDataIn } from './projectStore';
import { PlanRecordSchema, assertGenerationBase } from '../../app/presentation/planRecord';
...
async function collect(tx: IDBTransaction, project: Project) {
  const keepDecks = new Set([project.currentDeckId, project.previousDeckId, project.candidate?.deckId].filter(Boolean));
...
export const slidesStore: SlidesStore = {
  async open(id) {
    ...
    return (previous, next, record, options) =>
      transaction(
        ...
        async (tx) => {
          ...
          if (options?.isTaskActive && !options.isTaskActive()) throw new ContentError('inactive-edit', '修改已失效。');
