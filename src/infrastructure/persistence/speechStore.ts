...
export function createSpeechStore({ parsePlanRecord, assertGenerationBase }: PlanRecordContracts): SpeechStore {
  async function read(tx: IDBTransaction, projectId: string, preferPlan = false): Promise<SpeechWorkspace> {
    ...
    if (previous && previous.id !== project.previousDeckId)
      throw new ContentError('foreign-deck', '上一版身份不一致。');
    const base = {
