import type { ChatMessage } from './legacyMessage';
import type { Deck, RevisionRecord } from '../../modules/presentation/editing/schema';
...
export function assertMessage(message: ChatMessage, projectId: string, deck: Deck) {
  if (
    ...
    message.deckId !== deck.id ||
    message.baseRevision !== deck.revision ||
    !['user', 'assistant'].includes(message.role) ||
    !message.text.trim() ||
    !Number.isFinite(message.createdAt)
  )
    throw new Error('对话目标或内容无效');
}
export async function trimHistory(tx: IDBTransaction, projectId: string) {
  const history = ((await request(tx.objectStore('history').getAll())) as HistoryEntry[])
    .filter((item) => item.projectId === projectId)
    .sort((a, b) => b.createdAt - a.createdAt);
  // 请求标识单独保留，避免普通对话挤掉最近的修改去重记录。
  for (const items of [history.filter(isMessage), history.filter((item) => !isMessage(item))])
    items.slice(100).forEach((item) => {
