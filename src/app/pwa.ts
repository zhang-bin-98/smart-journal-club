...
function message<T>(worker: ServiceWorker, type: string): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = window.setTimeout(() => {
      channel.port1.close();
      reject(new Error('离线服务暂未响应，请稍后重试。'));
    }, 10000);
...
async function registerWorker() {
  ...
  const value = await navigator.serviceWorker.register(new URL('sw.js', root), {
    scope: root.pathname,
    updateViaCache: 'none',
  });
  registration = value;
...
export async function retryPwa() {
  ...
  try {
    if (!registration || (!registration.active && !registration.waiting && !registration.installing))
      ...
      if (registration.active && !registration.waiting && !registration.installing && !state.ready)
        await message(registration.active, 'CACHE_RESOURCES');
      await status();
    }
  } catch {
    change({ error: '暂时无法检查应用资源，请联网后重试。' });
  }
}
export async function installApp() {
  ...
  try {
    ...
    change({ error: '安装未完成，可以通过浏览器菜单安装应用。' });
  }
}
export async function applyAppUpdate() {
  ...
  try {
    ...
    if (!result.accepted) {
      acceptedUpdate = false;
      change({ updating: false, error: '请先关闭其他 smartJC 标签页，再更新此页面。' });
    }
  } catch (cause) {
    acceptedUpdate = false;
    change({ updating: false, error: cause instanceof Error ? cause.message : '更新失败，请重试。' });
  }
}
