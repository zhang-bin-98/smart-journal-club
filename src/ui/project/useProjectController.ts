...
export function useProjectController(opened: OpenProject, settings: ModelSettings, online: boolean, registerLeaveGuard?: RegisterLeaveGuard) {
  ...
  function changeInstruction(value: string) { instructionRef.current = value; setInstruction(value); setDirty(preferenceKey, value !== dataRef.current.project.preferences.instruction); }
  function commitInstruction() { void savePreferences().catch(cause => setError(errorMessage(cause))); }
  ...
  function cancelTask() { parseTask.current?.abort(); }
  ...
  return { data, instruction, changeInstruction, commitInstruction, source, openSource, regeneration, openRegeneration, error, busy, stage, warning, operationKind, session, image, persistRevision, generate, restore, cancelTask, exportPresentation, registerEditorLeave, resource };
}
