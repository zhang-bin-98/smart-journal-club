...
function ProjectContent({ opened, onLeave, settings, onSettings, registerLeaveGuard }: { opened: OpenProject; onLeave: () => void; settings: ModelSettings; onSettings: () => void; registerLeaveGuard?: RegisterLeaveGuard }) {
  ...
  const content = session ? <Editor key={session.current.id} session={session} readOnly={busy} resourceAvailable={!!resource} registerLeaveGuard={registerEditorLeave} onRegenerate={() => openRegeneration(true)} onRestore={data.project.previousDeckId ? restore : undefined} taskStatus={busy ? (operationKind === 'restore' ? '正在恢复上一版…' : `正在重生成：${stage || '规划汇报结构'}…`) : undefined} onCancelTask={operationKind === 'regenerate' ? cancelTask : undefined} externalError={error} paper={data.paper} image={image} name={data.project.name} initialSlideId={data.project.lastOpenedSlideId} onLeave={onLeave} onSettings={onSettings} aiSettings={settings} aiProjectId={data.project.id} aiPreferences={data.project.preferences} aiPersistRevision={persistRevision}
    ...
    onExport={exportPresentation} /> : <main className="mx-auto max-w-[1080px] px-5 py-6">
    ...
    <section className="mx-auto max-w-[760px] py-12">
      ...
      <textarea id="instruction" className={`${inputClass} mt-3 min-h-36 resize-y`} placeholder="例如：中文，15 页左右，重点讲结果和创新点" value={instruction} disabled={busy || data.project.checkpoint === 'deck-plan-ready'} onChange={event => changeInstruction(event.target.value)} onBlur={commitInstruction} />
      ...
      <div className="mt-5 flex flex-wrap items-center justify-end gap-3">
        {busy ? <><span role="status" className="text-sm text-muted">{stage}…</span><Button onClick={cancelTask}><X size={15} />取消</Button></>
