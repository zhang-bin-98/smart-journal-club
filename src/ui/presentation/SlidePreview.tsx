...
import { computeLayout, type Rect } from '../../modules/presentation/layout/computeLayout';
import type { Element, Slide } from '../../modules/presentation/editing/schema';
...
export function Editable({
  ...
  useLayoutEffect(() => {
    if (ref.current && !editing?.hasDraft(editKey) && ref.current.textContent !== value)
  ...
  const update = () => {
    ...
    editing?.onDraft({
      ...
      save: () => editing.onSave(editKey, text),
    });
  };
  return (
    <div
      ...
      onCompositionEnd={() => {
        ...
        update();
        if (document.activeElement !== ref.current) editing?.onBlur();
      }}
      onPaste={(event) => {
...
function Figure({ element, image }: { element: Extract<Element, { type: 'figure' }>; image: FigureImage }) {
  ...
  useEffect(() => {
    ...
    image(element).then(
      ...
      () => {
        if (active) setResult({ error: '图源不可用' });
      },
    );
    return () => {
      active = false;
  ...
  return result.url ? (
    <img className="block size-full object-contain" src={result.url} alt="论文图源" />
  ) : (
    <span className="text-xs text-muted">{result.error ?? '正在加载图源…'}</span>
  );
}
...
export function SlidePreview({
  ...
  return (
    <div
      ...
      className="relative aspect-video w-full overflow-hidden border border-control bg-white text-ink [container-type:inline-size] [&>*]:absolute"
    >
      <div
      ...
      {layout.message && (
        <div
          ...
          style={{
            ...position(layout.message),
            fontSize: `${layout.messageText.fontSize / 9.6}cqw`,
            lineHeight: layout.messageText.lineHeight,
          }}
        >
          <Editable value={slide.message ?? ''} editKey="message" label="幻灯片副标题" editing={editing} />
        </div>
      )}
        ...
        >
          {element.type === 'figure' ? (
            <button
              ...
              aria-label="选择 Figure"
              className="grid size-full grid-cols-1 grid-rows-1 cursor-pointer place-items-center bg-panel focus-visible:outline-2 focus-visible:outline-focus"
            >
              <Figure element={element} image={image} />
            </button>
          ) : element.type === 'citation' ? (
            <button
              type="button"
              tabIndex={thumbnail ? -1 : 0}
              className="block cursor-pointer text-left text-muted hover:underline"
              onClick={() => onSource?.(element.sourceIds[0])}
            >
              {sourceLabel(paper, element.sourceIds)}
            </button>
          ) : (
            <Editable
              value={element.type === 'text' ? element.text : element.items.join('\n')}
              editKey={element.id}
              label={element.type === 'text' ? '幻灯片文字' : '幻灯片列表'}
              editing={editing}
            />
          )}
        </div>
