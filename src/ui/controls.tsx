...
export const inputClass =
  'w-full rounded border border-control bg-white px-3 py-2 text-sm text-ink outline-none focus:border-accent focus:ring-1 focus:ring-focus disabled:opacity-45';
...
export function IconButton({
  ...
  return (
    <Button {...props} title={label} aria-label={label} className="size-9 p-2">
      {children}
    </Button>
  );
}
export function Brand() {
  return (
    <div className="flex shrink-0 items-center gap-2.5">
      <span className="grid size-8 place-items-center rounded bg-accent text-xs font-bold text-white">JC</span>
      <span className="text-lg font-semibold">smartJC</span>
    </div>
  );
}
