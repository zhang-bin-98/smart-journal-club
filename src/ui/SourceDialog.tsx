...
export function SourceDialog({
  ...
  const figure = paper.figures.find(
    (item) => item.sourceId === source?.id || item.panels.some((panel) => panel.sourceId === source?.id),
  );
  useEffect(() => {
      ...
      <div className="flex max-h-[94dvh] flex-col">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-line px-5 py-3">
          <h2 className="text-sm font-semibold">
            {figure?.label ?? '论文来源'} · 论文第 {source?.pageNumber ?? '?'} 页
          </h2>
          <IconButton label="关闭来源" disabled={saving} onClick={onClose}>
            <X size={16} />
          </IconButton>
        </header>
        <div className="min-h-0 overflow-auto bg-canvas p-3 sm:p-5">
          {!resource || !source || !page ? (
            ...
            <PdfPageView
              ...
              width={page.width}
              height={page.height}
              original={source.bbox}
              selected={box}
              onDraft={selection.onDraft}
              onBox={
                cropping && !saving && !readOnly
                  ? (next) => {
                      if (JSON.stringify(next) !== JSON.stringify(box)) selection.onDraft?.();
                      setBox(next);
                      setUseDefault(false);
                    }
                  : undefined
        ...
        <footer className="shrink-0 border-t border-line px-5 py-3">
          <p className="max-h-24 overflow-auto text-xs leading-relaxed wrap-anywhere text-muted">
            {figure?.caption ?? source?.textQuote}
          </p>
          {error && (
            ...
            {cropping && !readOnly ? (
              <>
                <Button
                  disabled={saving}
                  onClick={() => {
                    if (JSON.stringify(box) !== JSON.stringify(source?.bbox) || selection.element?.cropOverride)
                      selection.onDraft?.();
                    setBox(source?.bbox);
                    setUseDefault(true);
                  }}
                ...
                <Button
                  primary
                  disabled={!box || !resource || !selection.element || saving}
                  onClick={async () => {
                    if (!selection.element || !box || readOnly) return;
                    const done = beginActivity();
                    setSaving(true);
                    setError('');
                    const next = { ...selection.element };
                    if (useDefault) delete next.cropOverride;
                    else next.cropOverride = box;
                    try {
                      await selection.apply?.(next);
                      onClose();
                    } catch (cause) {
              ...
              selection.element &&
              !readOnly && (
                <Button disabled={!resource} onClick={() => setCropping(true)}>
                  <Crop size={15} />
                  调整裁图
                </Button>
              )
