    ...
    <div className="flex max-h-[94dvh] flex-col">
      ...
      <footer className="shrink-0 border-t border-line px-5 py-3">
        ...
        <div className="mt-3 flex flex-wrap justify-end gap-2">
          {cropping ? <><Button disabled={saving} onClick={() => { if (JSON.stringify(box) !== JSON.stringify(source?.bbox) || selection.element?.cropOverride) selection.onDraft?.(); setBox(source?.bbox); setUseDefault(true); }}><RotateCcw size={15} />恢复默认范围</Button><Button disabled={saving} onClick={onClose}>取消</Button><Button primary disabled={!box || !resource || !selection.element || saving} onClick={async () => {
