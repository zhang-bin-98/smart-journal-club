...
export type SourceSelection = { sourceId: string; element?: Extract<Element, { type: 'figure' }>; crop: boolean; apply?: (element: Extract<Element, { type: 'figure' }>) => Promise<void> };
export function SourceDialog({ paper, resource, selection, onClose }: { paper: Paper; resource?: PdfResource; selection: SourceSelection; onClose: () => void }) {
  ...
  return <dialog ref={dialog} aria-label="论文来源" onCancel={event => { event.preventDefault(); if (!saving) onClose(); }}
    ...
    <div className="flex max-h-[94dvh] flex-col">
      ...
      <div className="min-h-0 overflow-auto bg-canvas p-3 sm:p-5">
        {!resource || !source || !page ? <p role="alert" className="py-6 text-sm text-red-700">原 PDF 或来源缺失，无法查看及裁图。</p> : <PdfPageView resource={resource} pageNumber={page.pageNumber} width={page.width} height={page.height} original={source.bbox} selected={box} onBox={cropping && !saving ? next => { setBox(next); setUseDefault(false); } : undefined} />}
      </div>
      <footer className="shrink-0 border-t border-line px-5 py-3">
        ...
        <div className="mt-3 flex flex-wrap justify-end gap-2">
          {cropping ? <><Button disabled={saving} onClick={() => { setBox(source?.bbox); setUseDefault(true); }}><RotateCcw size={15} />恢复默认范围</Button><Button disabled={saving} onClick={onClose}>取消</Button><Button primary disabled={!box || !resource || !selection.element || saving} onClick={async () => {
            if (!selection.element || !box) return; setSaving(true); setError('');
            const next = { ...selection.element }; if (useDefault) delete next.cropOverride; else next.cropOverride = box;
            try { await selection.apply?.(next); onClose(); } catch (cause) { setError(errorMessage(cause)); } finally { setSaving(false); }
          }}>{saving ? '正在保存…' : '应用到本页'}</Button></> : selection.element && <Button disabled={!resource} onClick={() => setCropping(true)}><Crop size={15} />调整裁图</Button>}
        </div>
      </footer>
    </div>
  </dialog>;
