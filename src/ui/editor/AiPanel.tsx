...
export function AiPanel({ session, paper, settings, projectId, preferences, persistRevision, selectedSlideId, selectedElementId, onChanged, beforeSend, beforeUndo, onBusyChange, registerCancel, disabled = false }: {
  ...
  const controller = useAssistantController({ session, paper, settings, projectId, preferences, persistRevision, selectedSlideId, selectedElementId, onChanged, beforeSend, beforeUndo, onBusyChange, registerCancel, disabled });
  const { online, messages, pendingMessage, busy, loading, historyError, setHistoryAttempt, error, notice, input, changeInput, send, cancel, canUndo, undoRevision, messageList } = controller;
