...
export function useEditorController({
  ...
  const registerAiCancel = useCallback((cancel?: CancelAi) => {
    aiCancel.current = cancel;
  }, []);
  const aiBusyChanged = useCallback((busy: boolean) => {
    setAiBusy(busy);
    if (busy) setManualNotice('');
  }, []);
  const manualEdit = useCallback(() => {
    if (aiCancel.current?.('manual')) setManualNotice('已转为手工编辑');
  }, []);
  ...
  useEffect(() => {
    ...
    const beforeUnload = (event: BeforeUnloadEvent) => {
      if (draft.current || pending.current) event.preventDefault();
    };
    window.addEventListener('beforeunload', beforeUnload);
    return () => {
      active.current = false;
      setDirty(dirtyKey, false);
      setDirty(`${dirtyKey}-panels`, false);
  ...
  leave.current = async () => {
    try {
      if (readOnly) throw new Error('请先完成或取消当前任务');
  ...
  async function saveText(key: string, value: string) {
    ...
    if (key === 'title' || key === 'message')
      ...
      if (!item || (item.type !== 'text' && item.type !== 'bullet-list'))
        throw new Error('文字目标已变化，请重新打开项目');
      const next: Element = item.type === 'text' ? { ...item, text: value } : { ...item, items: value.split('\n') };
      await commit(
        { type: 'element', slideId: slide.id, elementId: item.id },
        [{ type: 'replace-element', slideId: slide.id, element: next }],
        '编辑文字',
      );
    }
  }
  async function flush() {
    ...
    try {
      ...
      setDirty(dirtyKey, !!draft.current);
      setStatus(draft.current ? '未保存' : '已保存');
      setError('');
    } catch (cause) {
      setStatus('未保存');
      throw cause;
  ...
  function source(sourceId?: string, crop = false) {
    ...
    if (!resourceAvailable) {
      setError('原 PDF 缺失，无法查看来源及裁图。');
      return;
    }
    if (crop && readOnly) return;
    const selectedFigure =
      element?.type === 'figure' ? paper.figures.find((item) => item.id === element.figureId) : undefined;
    const selectedSourceId =
      element?.type === 'figure' && element.panelId
        ? selectedFigure?.panels.find((panel) => panel.id === element.panelId)?.sourceId
        : selectedFigure?.sourceId;
    void run(() =>
      onSource?.(
        sourceId,
        element?.type === 'figure' && selectedSourceId === sourceId ? element : undefined,
        slide.id,
        crop,
        async (next) => {
          const current = session.current.slides
            .find((item) => item.id === slide.id)
            ?.elements.find((item) => item.id === next.id);
          if (current?.type !== 'figure' || current.figureId !== next.figureId || current.panelId !== next.panelId)
            throw new Error('图源已变化，请重新打开来源后裁图');
          const cropped = { ...current };
          if (next.cropOverride) cropped.cropOverride = next.cropOverride;
          else delete cropped.cropOverride;
          await commit(
            { type: 'element', slideId: slide.id, elementId: next.id },
            [{ type: 'replace-element', slideId: slide.id, element: cropped }],
            '调整当前元素裁图',
          );
        },
        manualEdit,
  ...
  async function addElement(type: 'text' | 'bullet-list' | 'citation') {
    ...
    const next: Element =
      type === 'text'
        ...
        : type === 'bullet-list'
          ? { id, type, items: [''] }
          : { id, type, sourceIds: slide.sourceIds.slice(0, 1) };
    await commit(
      { type: 'slides', slideIds: [slide.id] },
      [{ type: 'add-element', slideId: slide.id, element: next }],
      '新增元素',
    );
