...
export function HomePage({ openProject }: { openProject: (id: string) => void }) {
  ...
  useEffect(() => {
    let active = true;
    listProjects().then(result => { if (active) setProjects(result); }, cause => { if (active) setError(errorMessage(cause)); });
    return () => { active = false; };
  }, [refresh]);
  async function upload(files: FileList | File[]) {
    if (uploading.current) return;
    if (files.length !== 1) { setError('每个项目只接受一份 PDF'); return; }
    uploading.current = true; setSaving(true); setError('');
    try {
    ...
    finally { uploading.current = false; setSaving(false); if (fileInput.current) fileInput.current.value = ''; }
  }
  async function action(work: () => Promise<unknown>) {
    try { await work(); setMenu(undefined); setRename(undefined); setDeleting(undefined); setError(''); setRefresh(value => value + 1); }
    catch (cause) { setError(errorMessage(cause)); }
  }
  return <main className="mx-auto min-h-screen max-w-[1080px] px-5 py-6 text-ink">
    ...
    <section className="py-12 sm:py-16">
      ...
      <div className="mt-6 flex min-h-[220px] flex-col items-center justify-center gap-5 border-2 border-dashed border-control bg-white px-5 py-8" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); void upload(event.dataTransfer.files); }}>
        ...
        <Button primary disabled={saving} onClick={() => fileInput.current?.click()}><FileUp size={16} />{error ? '重新选择 PDF' : '选择 PDF'}</Button>
    ...
    {!!projects.length && <section><h2 className="mb-4 text-base font-semibold">最近项目</h2>
      <div className="border-t border-line">{projects.map(({ project, slideCount }) => <div key={project.id} className="relative flex flex-wrap items-center gap-3 border-b border-line py-4">
        {rename?.id === project.id ? <form className="flex min-w-0 flex-1 gap-2" onSubmit={event => { event.preventDefault(); if (rename.name.trim()) void action(() => updateProject(project.id, { name: rename.name.trim() })); }}>
          ...
          <Button type="submit" disabled={!rename.name.trim()}>保存</Button><Button onClick={() => setRename(undefined)}>取消</Button>
        </form> : <button className="min-w-0 flex-1 cursor-pointer text-left" onClick={() => openProject(project.id)}><span className="block truncate text-sm font-medium">{project.name}</span><span className="mt-1 block text-xs text-muted">{slideCount !== undefined ? `${slideCount} 页 · 已完成` : '待继续'} · {new Date(project.updatedAt).toLocaleString('zh-CN')}</span></button>}
        <IconButton label={`${project.name} 更多操作`} onClick={() => setMenu(menu === project.id ? undefined : project.id)}><MoreHorizontal size={16} /></IconButton>
        {menu === project.id && <div className="absolute top-14 right-0 z-10 grid gap-1 rounded border border-line bg-white p-1 shadow-sm"><Button onClick={() => { setRename({ id: project.id, name: project.name }); setMenu(undefined); }}><Pencil size={14} />重命名</Button><Button onClick={() => { setDeleting(project.id); setMenu(undefined); }}><Trash2 size={14} />删除项目</Button></div>}
        {deleting === project.id && <div role="alert" className="flex w-full flex-wrap items-center gap-3 text-sm text-red-700"><span>删除此项目及原 PDF？此操作无法撤销。</span><Button onClick={() => void action(() => deleteProject(project.id))}>确认删除</Button><Button onClick={() => setDeleting(undefined)}>取消</Button></div>}
      </div>)}</div>
    </section>}
  </main>;
}
