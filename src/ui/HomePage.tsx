...
export function HomePage({
  ...
  leave.current = async () => {
    if (uploading.current || working.current) throw new Error('正在保存项目，请稍后再离开');
  ...
  async function upload(files: FileList | File[]) {
    ...
    setError('');
    try {
      const { checkPdfFile } = await import('../shared/pdf/pdfResource');
      await checkPdfFile(files[0]);
  ...
  return (
    <main className="mx-auto min-h-screen max-w-[1080px] px-5 py-6 text-ink">
      ...
      <section className="py-12 sm:py-16">
        ...
        <div
          className="mt-6 flex min-h-[220px] flex-col items-center justify-center gap-5 border-2 border-dashed border-control bg-white px-5 py-8"
        ...
        >
          ...
          <Button primary disabled={saving || acting} onClick={() => fileInput.current?.click()}>
            <FileUp size={16} />
            {error ? '重新选择 PDF' : '选择 PDF'}
          </Button>
          <input
            ref={fileInput}
            type="file"
      ...
      {!!projects.length && (
        <section>
          <h2 className="mb-4 text-base font-semibold">最近项目</h2>
          <div className="border-t border-line">
            {projects.map(({ project, slideCount }) => (
              <div key={project.id} className="relative flex flex-wrap items-center gap-3 border-b border-line py-4">
                {rename?.id === project.id ? (
                  <form
                    ...
                    onSubmit={(event) => {
                      event.preventDefault();
                      if (rename.name.trim())
                        void action(() => updateProject(project.id, { name: rename.name.trim() }));
                    }}
                  >
                    <input
                      aria-label="项目名称"
                    ...
                    />
                    <Button type="submit" disabled={!rename.name.trim() || acting}>
                      保存
                    </Button>
                    <Button disabled={acting} onClick={closeForm}>
                      取消
                    </Button>
                  </form>
                ) : (
                  <button
                    disabled={saving || acting || !!rename || !!deleting}
                    className="min-w-0 flex-1 cursor-pointer text-left"
                    onClick={() => openProject(project.id)}
                  >
                    <span className="block truncate text-sm font-medium">{project.name}</span>
                    <span className="mt-1 block text-xs text-muted">
                      {slideCount !== undefined ? `${slideCount} 页 · 已完成` : '待继续'} ·{' '}
                ...
                <IconButton
                  ...
                  disabled={saving || acting || !!rename || !!deleting}
                  onClick={() => setMenu(menu === project.id ? undefined : project.id)}
                >
                  <MoreHorizontal size={16} />
                </IconButton>
                {menu === project.id && (
                  <div className="absolute top-14 right-0 z-10 grid gap-1 rounded border border-line bg-white p-1 shadow-sm">
                    <Button
                      onClick={() => {
                ...
                {deleting === project.id && (
                  <div role="alert" className="flex w-full flex-wrap items-center gap-3 text-sm text-red-700">
                    <span>删除此项目及原 PDF？此操作无法撤销。</span>
                    <Button disabled={acting} onClick={() => void action(() => deleteProject(project.id))}>
                      确认删除
                    </Button>
                    <Button disabled={acting} onClick={closeForm}>
                      取消
