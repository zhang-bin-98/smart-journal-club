...
export async function runAiRevision({ settings, paper, deck: inputDeck, session, request, selectedSlideId, selectedElementId, recentMessages = [], signal, projectId = '', requestId = crypto.randomUUID(), isTaskActive, preferences }: {
  ...
  const readDescriptions: Record<keyof typeof readSchemas, string> = { ...paperReadDescriptions, ...deckReadDescriptions };
  const readTools = Object.entries(readSchemas).map(([name, schema]) => tool(name, readDescriptions[name as keyof typeof readSchemas], schema));
  ...
  for (let turn = 0; turn < 10; turn++) {
    ...
    if (calls.length) {
      ...
      for (const call of calls) {
        ...
        } else if (Object.hasOwn(paperReadSchemas, call.name)) result = paperReadTool(call.name as keyof typeof paperReadSchemas, call.arguments, paper);
