...
function pageNumber(text: string) {
  ...
  for (const char of text) {
    if (char === '十' || char === '百') {
      total += (value || 1) * (char === '十' ? 10 : 100);
      value = 0;
    } else value = digits[char] ?? 0;
  }
  return total + value;
}
function explicitPages(request: string) {
  ...
  for (const match of request.matchAll(
    new RegExp(`第\\s*(${numberPattern})(?:\\s*[-—–到至]\\s*(${numberPattern}))?\\s*页|(?:slide|page)\\s*(\\d+)`, 'gi'),
  )) {
    const first = pageNumber(match[1] ?? match[3]);
    const last = match[2] ? pageNumber(match[2]) : first;
  ...
  if (leading) {
    const count = pageNumber(leading[1]);
    if (count > 100) return [0];
    for (let page = 1; page <= count; page++) pages.push(page);
  }
...
export function resolveSlideTarget(request: string, deck: Deck, selectedSlideId?: string) {
  ...
  return pages.length
    ? deck.slides[pages[0] - 1]?.id
    : (deck.slides.find((slide) => slide.id === selectedSlideId)?.id ?? deck.slides[0]?.id);
}
export function resolveAiTarget(
  ...
  const target: AiTarget = {
    slideIds: [],
    global,
    allowNewSlides: global || /新增|添加|插入|拆|分成|压成|合并|太挤|压到|增加.*页/.test(request),
  };
  if (!deck.slides.length && /这(?:一)?页|当前页|本页|这张图/.test(request))
    return { ...target, clarification: '当前文稿没有幻灯片，请先新增页面或明确要创建的内容。' };
  if (pages.some((page) => !deck.slides[page - 1]))
    return { ...target, clarification: '指定页码超出当前幻灯片范围，请提供有效页码。' };
  ...
  else if (/这(?:一)?页|当前页|本页/.test(request))
    target.slideIds = [resolveSlideTarget(request, deck, selectedSlideId)].filter((id): id is string => !!id);
  else {
    const sections = [
      { pattern: /背景/, kinds: ['background'] },
      { pattern: /研究问题/, kinds: ['question'] },
      { pattern: /方法/, kinds: ['method'] },
      { pattern: /结果部分|结果页/, kinds: ['result'] },
      { pattern: /讨论部分|讨论页/, kinds: ['discussion'] },
      { pattern: /结论部分|结论页/, kinds: ['conclusion'] },
    ];
  ...
  if (figureMatch) {
    ...
    if (figures.length !== 1)
      return { ...target, clarification: '无法唯一定位这个 Figure，请提供原论文图号及所在幻灯片页码。' };
    target.figureId = figures[0].id;
    let occurrences = deck.slides.filter((slide) =>
      slide.elements.some((element) => element.type === 'figure' && element.figureId === target.figureId),
    );
    if (target.slideIds.length) occurrences = occurrences.filter((slide) => target.slideIds.includes(slide.id));
      ...
      if (selected) occurrences = [selected];
        ...
        const previous = [...recentMessages]
          .reverse()
          .find((message) => message.targetSlideIds?.length && (!message.deckId || message.deckId === deck.id));
        const recent = occurrences.filter((slide) => previous?.targetSlideIds?.includes(slide.id));
        if (recent.length === 1) occurrences = recent;
      }
    }
    if (!occurrences.length)
      return { ...target, clarification: '指定范围中没有这个 Figure，请提供它所在的幻灯片页码。' };
    if (!target.slideIds.length && occurrences.length > 1)
      return {
        ...target,
        clarification: `这个 Figure 出现在第 ${occurrences.map((slide) => deck.slides.indexOf(slide) + 1).join('、')} 页，请指定要调整哪一处。`,
      };
    target.slideIds = occurrences.map((slide) => slide.id);
  }
  if (!target.slideIds.length && selectedSlideId && deck.slides.some((slide) => slide.id === selectedSlideId))
    target.slideIds = [selectedSlideId];
  if (!target.slideIds.length) {
    const previous = [...recentMessages]
      .reverse()
      .find(
        (message) =>
          message.targetSlideIds?.some((id) => deck.slides.some((slide) => slide.id === id)) &&
          (!message.deckId || message.deckId === deck.id),
      );
    target.slideIds =
      previous?.targetSlideIds?.filter((id) => deck.slides.some((slide) => slide.id === id)) ??
  ...
  if (
    ...
    if (selectedSlide?.elements.some((element) => element.id === selectedElementId))
      target.elementId = selectedElementId;
    else if (!selectedSlideId) {
      const previous = [...recentMessages]
        .reverse()
        .find(
          (message) => message.targetElementId && message.targetSlideIds?.some((id) => target.slideIds.includes(id)),
        );
      if (
        deck.slides.some(
          (slide) =>
            target.slideIds.includes(slide.id) &&
            slide.elements.some((element) => element.id === previous?.targetElementId),
        )
      )
        target.elementId = previous?.targetElementId;
    }
  }
  return target;
}
