...
export function applyMutation(
  ...
  if (mutation.type === 'add-slide') {
    ...
    deck.slides.splice(insertIndex(deck.slides, mutation.afterSlideId), 0, clone(mutation.slide));
    return { affected: [mutation.slide.id] };
  }
  ...
  if (mutation.type === 'move-slide') {
    ...
    deck.slides.splice(insertIndex(deck.slides, mutation.afterSlideId), 0, slide);
    return { affected: [mutation.slideId] };
  }
...
export function createSlide(id: string, number: number): Slide {
  return {
    id,
