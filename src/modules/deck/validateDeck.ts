...
export function validateDeck(input: unknown, paper?: Paper) {
  ...
  deck.slides.forEach(slide => {
    ...
    slide.elements.forEach(element => {
      ...
      if (paper && element.type === 'figure') { try { figureSource(paper, { figureId: element.figureId, panelId: element.panelId }); } catch (error) { errors.push(error instanceof Error ? error.message : String(error)); } }
