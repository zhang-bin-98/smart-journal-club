import { groupRects, containRect } from './index';
import { figureArea } from './figureGeometry';
import type { Element, Slide } from '../content/page';
...
export type ComputedLayout = {
  ...
  sourceLabel: Rect;
  elements: { element: Element; partitionRect: Rect; rect: Rect; text: TextMetrics }[];
};
...
function textMetrics(value: string, rect: Rect, preferred: number, minimum: number, lineHeight = 1.3): TextMetrics {
  const fits = (size: number) => {
    ...
    for (const char of value) {
      const next = size * ((char.codePointAt(0) ?? 0) > 0xff ? 1 : 0.56);
...
export function computeLayout(slide: Slide, aspects: Record<string, number> = {}): ComputedLayout {
  ...
  const partitions = slide.figureGroup ? groupRects(slide.figureGroup, figureArea(slide)) : undefined;
  const elements = content.map<{ element: Element; rect: Rect }>((element) => {
    if (element.type === 'figure' && partitions?.[element.id]) return { element, rect: partitions[element.id] };
  ...
  return {
    ...
    elements: elements.map((item) => ({
      ...item,
      partitionRect: item.rect,
      rect:
        item.element.type === 'figure' && aspects[item.element.id]
          ? containRect(item.rect, aspects[item.element.id])
          : item.rect,
      text: textMetrics(
