import type { Deck, DeckMutation, DeckSection, RevisionScope, Slide, SlideKind } from './schema';
import { layoutCapacity } from './layoutRules';
import { pruneGroup } from '../layout';
...
function fallbackLayout(slide: Slide) {
  ...
  const text = slide.elements.filter((e) => e.type === 'text' || e.type === 'bullet-list').length;
...
export function applyMutation(
  ...
  if (mutation.type === 'edit-speech' || mutation.type === 'split-speech') {
    const segment = deck.speech?.find((item) => item.id === mutation.segmentId);
    if (!segment) throw new Error('讲稿片段不存在。');
    const affected = deck.slides.filter((slide) => slide.speechIds?.includes(segment.id)).map((slide) => slide.id);
    if (mutation.type === 'edit-speech') segment.text = mutation.text;
    else {
      if (mutation.offset >= segment.text.length || deck.speech!.some((item) => item.id === mutation.newId))
        throw new Error('讲稿拆分位置或身份无效。');
      const paragraph = deck.speechParagraphs!.find((item) => item.id === segment.paragraphId)!;
      const next = { ...clone(segment), id: mutation.newId, text: segment.text.slice(mutation.offset) };
      segment.text = segment.text.slice(0, mutation.offset);
      deck.speech!.splice(deck.speech!.indexOf(segment) + 1, 0, next);
      paragraph.segmentIds.splice(paragraph.segmentIds.indexOf(segment.id) + 1, 0, next.id);
      for (const slide of deck.slides) {
        const at = slide.speechIds?.indexOf(segment.id) ?? -1;
        if (at >= 0) slide.speechIds!.splice(at + 1, 0, next.id);
      }
    }
    return { affected };
  }
    ...
    const at =
      deck.schemaVersion === 3
        ? mutation.afterSlideId === null
          ? 0
          : deck.slides.findIndex((item) => item.id === mutation.afterSlideId) + 1
        : insertIndexInSection(deck.slides, slide.sectionId, mutation.afterSlideId);
    if (mutation.afterSlideId !== null && at === 0) throw new Error('插入位置不存在');
  ...
  if (mutation.type === 'move-slide') {
    ...
    if (mutation.afterSlideId !== null) {
      ...
      if (deck.schemaVersion !== 3 && anchor.sectionId !== mutation.targetSectionId)
        throw new Error('插入位置不属于目标章节');
    }
  ...
  if (mutation.type === 'update-slide') {
    ...
    if (mutation.changes.figureGroup === null) delete slide.figureGroup;
    return { affected: [slide.id] };
  }
  ...
  if (slide.figureGroup)
    slide.figureGroup = pruneGroup(
      slide.figureGroup,
      slide.elements.filter((item) => item.type === 'figure').map((item) => item.id),
    );
