...
export async function parsePaper(resource: PdfResource, paper: Paper, signal: AbortSignal): Promise<Paper> {
  ...
  if (pages.reduce((total, page) => total + page.text.length, 0) < 100)
    throw new Error('未提取到足够文字，扫描件暂不支持，请更换可解析版本');
  const title = await resource.documentTitle();
  return {
    ...paper,
    metadata: { ...paper.metadata, ...(title ? { title } : {}) },
    pages,
    sources: pages
      ...
      .map((page) => ({
        ...
        textQuote: page.text.slice(0, 240),
      })),
  };
}
