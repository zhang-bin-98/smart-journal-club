...
const secondIntegrity = {
  ...integrity,
  'index.html': `sha256-${createHash('sha256').update(secondIndex).digest('base64')}`,
};
const secondWorker = workerSource
  .replace(/^(const VERSION = ")([^"]+)(";)/, '$1$2-test-update$3')
  .replace(/^const INTEGRITY = .+;$/m, `const INTEGRITY = ${JSON.stringify(secondIntegrity)};`);
let updateVersion = false;
let failInitialInstall = true;
const mime = {
  '.html': 'text/html',
  '.js': 'text/javascript',
  '.mjs': 'text/javascript',
...
const server = createServer(async (request, response) => {
  ...
  try {
    if (failInitialInstall && file === resolve(directory, blockedAsset)) {
      response.writeHead(404).end();
      return;
    }
    const content =
      file === join(directory, 'sw.js')
        ? updateVersion
          ? secondWorker
          : workerSource
        : file === join(directory, 'index.html') && updateVersion
          ? secondIndex
          : await readFile(file);
    response
      .writeHead(200, {
...
try {
  ...
  await page.evaluate(
    async ({ projectId, deck, paper, pdf, modelId }) => {
      const open = indexedDB.open('smartjc', 1);
      const db = await new Promise((resolve, reject) => {
        open.onsuccess = () => resolve(open.result);
        open.onerror = () => reject(open.error);
      ...
      const now = Date.now();
      const pdfAssetId = 'pwa-fixed-pdf';
      ...
      tx.objectStore('assets').put(
        {
          name: 'mechanism-modt-cdifficile.pdf',
          blob: new Blob([Uint8Array.from(atob(pdf), (char) => char.charCodeAt(0))], { type: 'application/pdf' }),
        },
        pdfAssetId,
      );
  ...
  await page.evaluate(
    async ({ version, asset }) => {
      const name = (await caches.keys()).find(
        (key) => key.startsWith('smartjc-static:') && key.endsWith(`:${version}`),
      );
  ...
  await page.evaluate(() => {
    window.__pwaNoReload = 'preserved';
  });
  ...
  const repair = await page.evaluate(async () => {
    ...
    return new Promise((resolve) => {
      ...
      registration.active.postMessage({ type: 'CACHE_RESOURCES' }, [channel.port2]);
    });
  });
  assert.equal(repair.updateRequired, true);
  const previousCache = await page.evaluate(
    async ({ version, asset }) => {
      const name = (await caches.keys()).find(
        (key) => key.startsWith('smartjc-static:') && key.endsWith(`:${version}`),
      );
      ...
      return {
        missing: !(await cache.match(new URL(asset, location.href))),
        index: await (await cache.match(new URL('index.html', location.href))).text(),
      };
    },
    { version: initialWorker.version, asset: blockedAsset },
  );
  ...
  assert.equal(
    await second.getByRole('textbox', { name: '幻灯片标题', exact: true }).innerText(),
    '另一标签页未保存草稿',
  );
  await second.close();
  ...
  assert.equal(
    await page.evaluate(async () => {
      ...
      const urls = (
        await Promise.all(
          keys.map(async (key) => (await (await caches.open(key)).keys()).map((request) => request.url)),
        )
      ).flat();
  ...
  console.log(
    'PASS: update waiting/no automatic reload/dirty and busy guard/other tab preserved/user accepted reload/IDB and unrelated cache preserved/API uncached',
  );
