...
try {
  ...
  const checkpoint = await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).project.checkpoint, id);
  ...
  const generated = await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id);
  ...
  const saved = await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id);
  ...
  assert.equal(await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck.revision, id), 1);
  ...
  const beforeAi = await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id);
  ...
  const afterAi = await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id);
  ...
  await page.waitForFunction(async ({ id, revision }) => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck.revision === revision, { id, revision: afterAi.revision + 1 });
  assert.equal((await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id)).slides[0].title, beforeAi.slides[0].title);
  ...
  const beforeCancelledAi = await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id);
  aiMode = 'held';
  ...
  assert.equal((await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id)).revision, beforeCancelledAi.revision);
  await heldAiRoute.abort().catch(() => {});
  await aiInput.click();
  await page.getByRole('status').filter({ hasText: '已保存' }).waitFor();
  const afterCancelledAi = await page.evaluate(async id => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).deck, id);
  ...
  assert.equal((await page.evaluate(async id => (await import('/src/modules/assistant/conversationRepository.ts')).loadHistory(id), id)).length, 4);
  await page.unroute('https://api.deepseek.com/chat/completions');
  console.log('PASS: AI send/explicit target/summary top undo/manual draft cancels staged candidate/history reopen');
  const beforeRegeneration = await page.evaluate(async id => (await import('/src/modules/project/projectRepository.ts')).loadProject(id), id);
  const regenerationCalls = []; let holdRegeneration = true; let heldRegenerationRoute; let markRegenerationHeld;
  const regenerationHeld = new Promise(resolve => { markRegenerationHeld = resolve; });
  await page.route('https://api.deepseek.com/chat/completions', async route => {
    const request = route.request().postDataJSON();
    const content = request.messages.find(message => message.role === 'user').content;
    const data = JSON.parse(typeof content === 'string' ? content : content.find(item => item.type === 'text').text);
    const stage = data.plan ? 'generate' : 'plan'; regenerationCalls.push(stage);
    if (stage === 'generate' && holdRegeneration) { heldRegenerationRoute = route; markRegenerationHeld(); return; }
    const result = await page.evaluate(async ({ stage, data }) => {
      const fixed = await import('/tests/generation-contracts.ts');
  ...
  await beginRegeneration(); await regenerationHeld;
  assert.equal(await page.locator('[data-slide-preview=current] [data-edit-key=title]').getAttribute('contenteditable'), 'false');
  ...
  let versionData = await page.evaluate(async id => (await import('/src/modules/project/projectRepository.ts')).loadProject(id), id);
  ...
  await page.waitForFunction(async ({ id, previous }) => (await (await import('/src/modules/project/projectRepository.ts')).loadProject(id)).project.currentDeckId !== previous, { id, previous: beforeRegeneration.deck.id });
  await page.getByRole('button', { name: '取消重生成', exact: true }).waitFor({ state: 'hidden' });
  versionData = await page.evaluate(async id => (await import('/src/modules/project/projectRepository.ts')).loadProject(id), id);
  ...
  await page.getByRole('button', { name: '更多操作', exact: true }).click();
  const restoredVersion = await page.evaluate(async id => (await import('/src/modules/project/projectRepository.ts')).loadProject(id), id);
  ...
  await page.unroute('https://api.deepseek.com/chat/completions');
  console.log('PASS: regenerate only plan/generate/cancel preserves current/success preferences/restore/reopen/clear undo');
