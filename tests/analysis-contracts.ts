...
export function understanding(paper: Paper) {
  ...
  return {
    ...
    story: {
      ...
      question: [],
      studyDesign: [],
      mainFindings: [{ text: '固定发现', claimIds: ['claim-temp'], sourceIds: [] }],
      novelty: [],
      limitations: [],
      conclusion: [],
    },
...
export async function runAnalysisContracts() {
  ...
  const raw = understanding(paper);
  const result = mapUnderstanding(paper, raw);
  ...
  assert(
    (await loadProject(project.id)).paper.claims[0].id === result.paper.claims[0].id,
    '非法结果不得覆盖稳定 Paper',
  );
  await deleteProject(project.id);
  const files: Record<string, string> = {
    'prompts/common.md': prompts.common,
    ...Object.fromEntries(Object.entries(prompts.stages).map(([id, body]) => [`prompts/stages/${id}.md`, body])),
    ...Object.fromEntries(
      prompts.strategies.map((strategy) => [
        `prompts/research/${strategy.id}.md`,
  ...
  const missingGeneral = { ...files };
  delete missingGeneral['prompts/research/general.md'];
