...
async function database(work: (tx: IDBTransaction) => void) {
  await new Promise<void>((resolve, reject) => {
    ...
    request.onsuccess = () => {
      const db = request.result; const tx = db.transaction(['projects', 'papers', 'decks', 'settings'], 'readwrite');
      work(tx); tx.oncomplete = () => { db.close(); resolve(); }; tx.onabort = () => { db.close(); reject(tx.error); };
    };
...
export async function runContracts() {
  ...
  IDBObjectStore.prototype.add = function (...args) { if (this.name === 'assets') throw new DOMException('fixed failure', 'QuotaExceededError'); return originalAdd.apply(this, args); };
  try { await rejected(() => createProject(new File(['%PDF-fixture'], 'atomic.pdf')), '创建失败必须拒绝'); }
  ...
  await rejected(() => saveStage(project, { checkpoint: 'figures-ready', paper }, cancelled.signal), '取消不得推进阶段');
  ...
  IDBObjectStore.prototype.put = function (...args) { if (this.name === 'decks') throw new DOMException('fixed failure', 'QuotaExceededError'); return originalPut.apply(this, args); };
  try {
  ...
  await session.undo(); assert(session.current.revision === 2 && session.current.slides[0].title === initial.slides[0].title, 'Undo 恢复内容并递增版本');
  await session.redo(); assert(session.current.revision === 3, 'Redo 递增版本');
  await session.undo(); await edit('撤销后新编辑'); assert(!session.canRedo, '新编辑清空 Redo');
  const stable = JSON.stringify(session.current);
  await rejected(() => session.commit({ type: 'slides', slideIds: ['slide-1'] }, [
  ...
  await session.commit({ type: 'element', slideId: 'slide-2', elementId: figure.id }, [{ type: 'replace-element', slideId: 'slide-2', element: { ...figure, cropOverride: { x: .1, y: .1, width: .5, height: .5 } } }], '裁图');
  const restored = await loadProject(project.id);
  assert(restored.deck!.slides[2].elements.every(element => element.type !== 'figure' || !element.cropOverride), '裁图不得影响共享引用');
  assert(JSON.stringify(restored.paper.sources) === JSON.stringify(paper.sources), '裁图不得改 Source');
  await rejected(() => session.commit({ type: 'element', slideId: 'slide-2', elementId: figure.id }, [{ type: 'replace-element', slideId: 'slide-2', element: { ...figure, cropOverride: { x: .9, y: .1, width: .5, height: .5 } } }], '越界裁图'), 'bbox 越界必须拒绝');
  const broken = structuredClone(paper); broken.figures[0].panels[0].sourceId = 'missing';
  ...
  await database(tx => {
    const req = tx.objectStore('settings').get('contract-check'); req.onsuccess = () => { assert(req.result?.marker === 'independent-setting', '删除项目不得删除设置'); tx.objectStore('settings').delete('contract-check'); };
  });
