...
async function runAiModelContracts() {
  ...
  assert(
    resolveAiTarget('Figure 3 只留 A', deck, paper, 'slide-1').clarification,
  ...
  const intent = (mode: 'answer' | 'revision') => () =>
    call('submit_result', { result: { mode, needsStrategy: false } });
  const run = (session: DeckSession, signal = new AbortController().signal, request = '第 1 页标题短一点') =>
    runAiRevision({
      settings,
      paper,
  ...
  const readRequests = await fixedResponses(
    [intent('answer'), () => call('paper.get_page', { pageNumber: 1 }), () => answer('图源可以在原文第一页核对。')],
    () => run(readSession, undefined, '检查这页的证据'),
  );
  assert(
    readRequests
      .slice(1)
      .every((request) => !request.tools.some((tool) => tool.function.name === 'deck__apply_revision')),
    '问答回合不得开放写工具',
  );
  assert(
    readRequests[2].messages.some(
      (message) => message.role === 'tool' && message.content?.includes('Fixture source page'),
    ),
    '读取工具必须使用当前论文数据',
  );
  assert(readSession.current.revision === 0 && !readSession.canUndo, '问答和读工具不能写入');
  await fixedResponses([intent('answer'), () => call('deck.apply_revision', local)], () =>
    rejected(() => run(readSession, undefined, '检查标题'), '问答中伪造写工具调用必须拒绝'),
  );
  assert(readSession.current.revision === 0, '问答伪造写调用不得提交');

  const success = new DeckSession(deck, paper);
  let stagedRevision = -1;
  const titleRequests = await fixedResponses(
    [
      intent('revision'),
      () => call('deck.apply_revision', local),
      () => {
        stagedRevision = success.current.revision;
        return answer('已精简第一页标题。');
      },
    ],
    () => run(success, undefined, titleRequest),
  );
  const writeScope = titleRequests[1].tools.find((tool) => tool.function.name === 'deck__apply_revision')?.function
