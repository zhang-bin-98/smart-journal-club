...
async function rejected(work: () => unknown | Promise<unknown>, message: string) {
  let failed = false; try { await work(); } catch { failed = true; } assert(failed, message);
}
const batch: ApplyRevisionArgs = {
  scope: { type: 'slides', slideIds: ['slide-1', 'slide-2'] }, summary: '同时精简两页标题',
  mutations: [
...
export async function runAiContracts() {
  ...
  try {
    ...
    await session.undo();
    assert(session.current.revision === 2 && session.current.slides.every((slide, index) => slide.title === deck.slides[index].title) && !session.canUndo, '一次 Undo 应恢复整批内容并递增 revision');
    await rejected(() => session.applyRevision(capture(project.id, deck), batch), '绑定旧版本的 AI 候选不得提交');

    const stable = JSON.stringify(session.current); const historyCount = (await loadHistory(project.id)).length;
    const nextMessages = messages.map(message => ({ ...message, id: crypto.randomUUID(), baseRevision: session.current.revision, ...(message.revision === undefined ? {} : { revision: session.current.revision + 1 }) }));
    ...
    const controller = new AbortController(); const originalPut = IDBObjectStore.prototype.put;
    ...
    let active = true;
    IDBObjectStore.prototype.put = function (...args) {
      const result = originalPut.apply(this, args); if (this.name === 'decks') active = false; return result;
    };
    ...
    await rejected(() => session.applyRevision({ ...capture(project.id, session.current), requestId: request.requestId }, batch), '新增对话不得挤掉请求去重记录');
  } finally { await deleteProject(project.id); }
  const remaining = await new Promise<number>((resolve, reject) => {
    const request = indexedDB.open('smartjc', 1);
    request.onsuccess = () => {
      const db = request.result; const tx = db.transaction('history'); const all = tx.objectStore('history').getAll();
...
async function fixedResponses(steps: (() => Response | Promise<Response>)[], work: () => Promise<unknown>) {
  const originalFetch = globalThis.fetch; const requests: WireRequest[] = []; let used = 0;
  globalThis.fetch = async (input, init) => {
    ...
    const step = steps[used++]; if (!step) throw new Error('固定响应已用尽'); return step();
  };
  try { await work(); assert(used === steps.length, '应实际执行所有固定工具回合'); return requests; }
  finally { globalThis.fetch = originalFetch; }
}
async function runAiModelContracts() {
  const deck = structuredClone(fixtureDeck); const paper = structuredClone(fixturePaper);
  const titleRequest = '请把第1页标题改为“酵母交配信号重新激活 TORC1”，只修改第1页标题，其他内容保持原样。';
  assert(resolveAiTarget(titleRequest, deck, paper, 'slide-3').titleOnly === true, '保留其他内容的约束不得扩大标题修改范围');
  assert(resolveAiTarget('只修改第1页标题，不要修改其他内容', deck, paper, 'slide-3').titleOnly === true, '否定其他内容修改时仍是标题请求');
  ...
  assert(resolveAiTarget('第一至二页标题短一点', deck, paper, 'slide-3').slideIds.join() === 'slide-1,slide-2', '中文连续页码应按当前页序定位');
  assert(resolveAiTarget('第 99 页标题短一点', deck, paper, 'slide-1').clarification, '越界页码应澄清而不是回退到当前页');
  assert(resolveAiTarget('Figure 3 只留 A', deck, paper, 'slide-1').clarification, '重复 Figure 没有可消歧选择时应澄清');
  assert(resolveAiTarget('这页简短一些', deck, paper, 'slide-2', undefined, [{ role: 'assistant', text: '上次建议', targetSlideIds: ['slide-1'] }]).slideIds.join() === 'slide-2', '这页必须使用当前选择');
  assert(resolveAiTarget('按刚才建议修改', deck, paper, undefined, undefined, [{ role: 'assistant', text: '上次建议', targetSlideIds: ['slide-2'] }]).slideIds.join() === 'slide-2', '没有当前选择时应使用最近目标');
  const local: ApplyRevisionArgs = { scope: { type: 'slides', slideIds: ['slide-1'] }, summary: '精简标题', mutations: [batch.mutations[0]] };
  await rejected(() => validateAiCandidate({ ...local, scope: { type: 'deck' } }, target, deck, paper), '局部请求不能用 deck scope 绕过范围检查');
  await rejected(() => validateAiCandidate({ ...local, scope: { type: 'element', slideId: 'slide-1', elementId: 't1' } }, target, deck, paper), '页面标题的 update-slide 不能以 element 范围提交');
  await rejected(() => validateAiCandidate({ ...local, mutations: [batch.mutations[1]] }, target, deck, paper), '局部请求不得改选中页来替代显式页码');
  await rejected(() => validateAiCandidate({ ...local, mutations: [{ type: 'replace-element', slideId: 'slide-1', element: { id: 't1', type: 'text', text: '偷偷改正文' } }] }, target, deck, paper), '标题微调不能扩散到正文');
  const settings = { ...DEFAULT_SETTINGS, apiKey: 'fixed-test-key' };
  const intent = (mode: 'answer' | 'revision') => () => call('submit_result', { result: { mode, needsStrategy: false } });
  const run = (session: DeckSession, signal = new AbortController().signal, request = '第 1 页标题短一点') => runAiRevision({ settings, paper, deck: session.current, session, signal, request, selectedSlideId: 'slide-2', selectedElementId: 'f1' });

  const readSession = new DeckSession(deck, paper);
  const readRequests = await fixedResponses([intent('answer'), () => call('paper.get_page', { pageNumber: 1 }), () => answer('图源可以在原文第一页核对。')], () => run(readSession, undefined, '检查这页的证据'));
  assert(readRequests.slice(1).every(request => !request.tools.some(tool => tool.function.name === 'deck__apply_revision')), '问答回合不得开放写工具');
  assert(readRequests[2].messages.some(message => message.role === 'tool' && message.content?.includes('Fixture source page')), '读取工具必须使用当前论文数据');
  assert(readSession.current.revision === 0 && !readSession.canUndo, '问答和读工具不能写入');
  await fixedResponses([intent('answer'), () => call('deck.apply_revision', local)], () => rejected(() => run(readSession, undefined, '检查标题'), '问答中伪造写工具调用必须拒绝'));
  assert(readSession.current.revision === 0, '问答伪造写调用不得提交');

  const success = new DeckSession(deck, paper); let stagedRevision = -1;
  const titleRequests = await fixedResponses([intent('revision'), () => call('deck.apply_revision', local), () => { stagedRevision = success.current.revision; return answer('已精简第一页标题。'); }], () => run(success, undefined, titleRequest));
  const writeScope = titleRequests[1].tools.find(tool => tool.function.name === 'deck__apply_revision')?.function.parameters?.properties?.scope;
  assert(writeScope?.properties?.type?.const === 'slides' && !JSON.stringify(writeScope).includes('element') && writeScope.properties.slideIds.items?.enum?.join() === 'slide-1', '标题请求的写工具只应提供绑定目标页的 slides 范围');
  assert(stagedRevision === 0 && success.current.revision === 1 && success.current.slides[0].title === '第一处 AI 修改' && success.current.slides[1].title === deck.slides[1].title, '工具仅暂存，正常结束后才修改显式目标');
  await success.undo(); assert(!success.canUndo && success.current.slides[0].title === deck.slides[0].title, '一次 AI 请求只产生一次 Undo');
  for (const failure of ['network', 'cancel', 'duplicate', 'stale'] as const) {
    const session = new DeckSession(deck, paper); const controller = new AbortController();
    await fixedResponses([intent('revision'), () => call('deck.apply_revision', local), async () => {
      assert(session.current.revision === 0, '失败前候选不能提前提交');
      if (failure === 'network') return new Response('{"error":{"message":"fixed failure"}}', { status: 500, headers: { 'Content-Type': 'application/json' } });
